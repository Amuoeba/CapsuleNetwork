...
class myNORBreader(Dataset):
    """
    myNORBreader reads the small NORB data which is given in the "your_dir"/Processed/Train and "your_dir"/Processed/Test directories.
    The data is in the form of SmallNORBExample objects, which were obtained from raw NORB data provided on the web.
    Args:
        - root: the root directory fro your data
        - train: True/False depending on wheter you want train or test set
        - transform: TODO implement transforms, curently defaults to False
    Returns:
        - implements __len__(self) and __getitem__(self,idx) methods which are then used by
          myNORBloader to shufle and load data in batches
    """
